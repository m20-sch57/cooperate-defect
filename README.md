...
# Ограничения
Стратегия должна быть исполняемым файлом под Ubuntu >= 22.04 (например, питоновским скриптом). В рамках одного матча время работы каждой стратегии не должно превышать 0.25 сек; объём потребляемой памяти — 512MB.
